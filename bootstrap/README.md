...
| `-RootPassword` | (trống → sinh ngẫu nhiên) | Mật khẩu root; để trống sẽ tự sinh và in ra cuối |
| `-UserPassword` | (trống → sinh ngẫu nhiên) | Mật khẩu user `hostvn`; để trống sẽ tự sinh và in ra cuối |
...
# mật khẩu: được SINH NGẪU NHIÊN mỗi lần cài và IN RA ở cuối quá trình (mục
#           "LUU LAI THONG TIN DANG NHAP") — hãy lưu lại ngay.
# trong container:  hostvn  (mật khẩu user cũng sinh ngẫu nhiên, in cùng chỗ)
```
Vẫn hỗ trợ đăng nhập bằng key: `ssh -i ..\.ssh\id_ed25519 -p 2233 root@<IP>`

> ℹ️ Không còn mật khẩu mặc định cố định. Mỗi lần chạy bootstrap sinh mật khẩu root/user
> ngẫu nhiên (crypto RNG) và in ra một lần để bạn lưu — **không lưu tự động ở đâu cả**.
> Muốn dùng mật khẩu cố định của riêng bạn: `-RootPassword '...'` (và `-UserPassword '...'`)
> khi chạy. Đổi bất cứ lúc nào sau khi vào: `passwd root`.
